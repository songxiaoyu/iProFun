...
iProFun_detection=function(reg.all, eFDR.all, FWER.all, filter=c(0, 0), NoProbButFWERIndex=NULL,
                  fdr.cutoff = 0.1, fwer.cutoff=0.1, PostPob.cutoff=0.75, var.ID,
                  xType, yType) {
  
  reg.sum=multi.omic.reg.summary(reg.out.list=reg.all, var.ID=var.ID)
  
  xlength=length(reg.all)
  ...
  for (p in x.fwer.idx) {
    ...
    if (is.null(filter[p]) ){ # no requirement
      x_filter_gene=seq(1, nrow(betas_J))
    } else {
